...
workspace/
  ...
  .Codex/           # workspace 级 AI 配置（AGENTS.md, skills/）
```
...
| `ai_processor.rs` | AI 处理队列消费者，workspace `.Codex/` 初始化，事件发射 |
...
| `skills.rs` | 技能插件发现（项目级 + 全局 `~/.Codex/skills/`），`SKILL.md` frontmatter 解析 |
