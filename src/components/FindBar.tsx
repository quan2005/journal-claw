...
export function FindBar({ containerRef, onClose }: FindBarProps) {
  ...
  useEffect(() => { updateHighlights() }, [updateHighlights])
  ...
  return (
    <div style={{
      ...
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--item-meta)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, opacity: 0.45 }}>
      ...
      <NavBtn onClick={goPrev} disabled={matches.length === 0} title={t('findPrev')} direction="up" />
      <NavBtn onClick={goNext} disabled={matches.length === 0} title={t('findNext')} direction="down" />
      <button
        onClick={onClose}
        title={t('closeFindBar')}
        style={{
          background: 'transparent', border: 'none', cursor: 'pointer',
          padding: 2, display: 'flex', alignItems: 'center', opacity: 0.35,
...
function NavBtn({ onClick, disabled, title, direction }: { onClick: () => void; disabled: boolean; title: string; direction: 'up' | 'down' }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      ...
      onMouseLeave={e => { if (!disabled) (e.currentTarget as HTMLButtonElement).style.opacity = '0.5' }}
    >
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--item-meta)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        {direction === 'up'
