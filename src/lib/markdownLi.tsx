...
const checkboxWrapStyle: React.CSSProperties = { flexShrink: 0, width: 20, display: 'inline-flex', justifyContent: 'center', marginTop: 7 }
const bulletWrapStyle: React.CSSProperties = { flexShrink: 0, width: 20, display: 'inline-flex', justifyContent: 'center', marginTop: 12 }
const bulletStyle: React.CSSProperties = { width: 4, height: 4, borderRadius: '50%', backgroundColor: 'var(--md-bullet)' }
...
export function MarkdownLi({ children, ...liProps }: React.ComponentProps<'li'> & { ordered?: boolean; className?: string }) {
  ...
  if (isTask) {
    ...
    return (
      <li style={{ fontSize: 'var(--text-md)', color: isChecked ? 'var(--md-checkbox-done-text)' : 'var(--md-text)', lineHeight: 1.75, display: 'flex', alignItems: 'flex-start', listStyle: 'none', textDecoration: isChecked ? 'line-through' : 'none' }}>
        <span style={checkboxWrapStyle}><Checkbox checked={isChecked} /></span>
  ...
  return (
    <li style={{ fontSize: 'var(--text-md)', color: 'var(--md-text)', lineHeight: 1.75, display: 'flex', alignItems: 'flex-start' }}>
      <span style={bulletWrapStyle}><span style={bulletStyle} /></span>
      <span>{children}</span>
    </li>
  )
}
