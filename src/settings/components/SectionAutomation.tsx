...
export default function SectionAutomation() {
  ...
  useEffect(() => {
    Promise.all([getAutoLintConfig(), getAutoLintStatus()])
      .then(([cfg, st]) => {
        setConfig(cfg)
        setStatus(st)
        setLoading(false)
  ...
  const updateConfig = useCallback(
    async (patch: Partial<AutoLintConfig>) => {
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--duration-text)', marginTop: 2 }}>
                {t('autoLintDesc')}
              </div>
            </div>
            {/* Toggle */}
          ...
          {config.enabled && (
            <>
              ...
              <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 11, color: 'var(--duration-text)', marginBottom: 6 }}>
                  {t('frequency')}
                </div>
                <Segment
                  options={freqOptions}
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <Segment
                    ...
                    value={config.min_entries}
                    onChange={(v) => updateConfig({ min_entries: v })}
                  />
                  <span style={{ fontSize: 11, color: 'var(--duration-text)' }}>
                    {t('entries')} · {t('skipIfInsufficient')}
                  </span>
                </div>
              </div>
                ...
                <div>
                  {status?.state === 'running' ? (
                    ...
                    <>
                      {status?.last_run && (
                        <div
                          style={{ fontSize: 11, color: 'var(--duration-text)', marginBottom: 3 }}
                        >
                          {t('lastRun')}: {status.last_run}
                          {status.last_run_entries != null &&
                            ` · ${t('organized')} ${status.last_run_entries} ${t('entries')}`}
                        </div>
                      )}
                      {status?.next_check && (
                        <div style={{ fontSize: 11, color: 'var(--record-btn)' }}>
                          {t('nextCheck')}: {status.next_check} · {t('currentNew')}{' '}
                          {status.current_new_entries} {t('entries')}
                        </div>
                      )}
                    </>
                  )}
