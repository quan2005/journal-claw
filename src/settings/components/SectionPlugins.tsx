...
export default function SectionPlugins() {
  ...
  return (
    <div style={sectionStyle}>
      <div style={{ fontSize: 13, color: 'var(--month-label)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 16, fontWeight: 500 }}>
        {t('pluginsSection')}
      </div>
      ...
      {loading ? null : skills.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, animation: 'section-fadein 160ms ease-out both' }}>
          {projectSkills.length > 0 && (
            <SkillGroup label={t('pluginScopeProject')} skills={projectSkills} onOpenDir={() => openSkillsDir('project')} />
          )}
          {globalSkills.length > 0 && (
            <SkillGroup label={t('pluginScopeGlobal')} skills={globalSkills} onOpenDir={() => openSkillsDir('global')} />
          )}
        </div>
      )}
    </div>
...
function SkillGroup({ label, skills, onOpenDir }: {
  label: string
  skills: SkillInfo[]
  onOpenDir: () => void
}) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        <div style={{ fontSize: 11, color: 'var(--duration-text)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
          {label}
        </div>
