...
export default function SectionPlugins() {
  return (
    <div style={sectionStyle}>
      <div style={{ fontSize: 13, color: 'var(--month-label)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 16, fontWeight: 500 }}>技能插件</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {PLUGINS.map(({ icon, iconBg, name, desc }) => (
          <div key={name} style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, color: 'var(--item-text)', marginBottom: 2 }}>{name}</div>
              <div style={{ fontSize: 12, color: 'var(--duration-text)', lineHeight: 1.4 }}>{desc}</div>
            </div>
            <div style={{ fontSize: 11, color: 'var(--record-btn)', background: 'rgba(200,147,58,0.08)', border: '1px solid rgba(200,147,58,0.15)', borderRadius: 4, padding: '2px 7px', whiteSpace: 'nowrap' }}>即将推出</div>
          </div>
        ))}
        <div style={{ border: '1px dashed var(--divider)', borderRadius: 8, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12, opacity: 0.4 }}>
          ...
          <div>
            <div style={{ fontSize: 14, color: 'var(--item-meta)' }}>更多插件</div>
            <div style={{ fontSize: 12, color: 'var(--duration-text)' }}>插件市场即将开放</div>
          </div>
        </div>
      </div>
    </div>
