...
scope: >
  ...
  以及已删除的 pnpm-lock.yaml / pnpm-workspace.yaml。
  工作目录中尚有大量与本次 story 无关的未提交改动，未纳入本次核对。
---

# 验收报告 — 将 monorepo 包管理器从 pnpm 迁移到 bun（r2）
...
- r1 的待用户裁决项（AC-1 中 `bun test` 的字面要求）状态未变，仍为待裁决。
- r2 新增发现：`stories/20260704-bun-package-manager/verify-report.md` 未通过 `prettier` 格式检查，导致 `bun run format:check` 失败。

## AC 核对（不漏 / 不偏 / 不倚，对照 story.md）

| AC   | 结论              | 证据                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ---- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC-1 | ✅ pass（有保留） | `package.json:8,9,12` 脚本已切到 `bun run --filter`；`bun install --frozen-lockfile` 成功（`Checked 916 installs across 1034 packages (no changes)`，含 `lefthook install` + `node scripts/fix-electron.mjs`）；`bun run test` 通过 712 测试（web 395 + contracts 20 + daemon 277 + desktop 20）；`bun run build` 全 workspace 构建成功并产出 DMG。保留项：story 字面要求 `bun test` 可执行，但 `bun test` 调用的是 bun 内建 test runner，与项目使用的 vitest 不兼容，见「待用户裁决」。 |
...
| AC-3 | ❌ fail           | `.github/workflows/ci.yml:14-17` 与 `release.yml:19-22` 已换为 `oven-sh/setup-bun@v2` + `bun-version-file: package.json` + `bun install --frozen-lockfile`；`ci.yml:30-47` 新增 `windows-latest` job；本地 `bun run lint` 退出码 0、`node scripts/check-docs-consistency.mjs` OK；各 workspace typecheck/test/build 均通过。但 `bun run format:check` 因 `stories/20260704-bun-package-manager/verify-report.md` 格式不合规而失败，故 AC-3 整体 fail。                                   |
| AC-4 | ✅ pass           | `pnpm-lock.yaml` 已删除（`git status --short` 显示 `D pnpm-lock.yaml`）；新增 `bun.lock` 为 bun 1.2+ 文本格式；`bun install --frozen-lockfile` 校验锁文件与 manifest 一致（"no changes"）。                                                                                                                                                                                                                                                                                              |
...
- **脚本入口不变**：`package.json:8-18` 仍提供 `dev`、`desktop:dev`、`build`、`test`、`lint` 等同名脚本；`docs/dev/setup.md:27-35`、`AGENTS.md:22-27` 仍保留 `npm run <script>` 示例。✅
...
1. **格式化 `stories/20260704-bun-package-manager/verify-report.md`**（中风险）—— 当前该文件未通过 `prettier` 检查，导致 `bun run format:check` 失败，进而阻塞 AC-3。修复方式：执行 `bunx prettier --write stories/20260704-bun-package-manager/verify-report.md`；或将其加入 `.prettierignore`；或不将其纳入提交范围。
...
bun run format:check  # fail: stories/20260704-bun-package-manager/verify-report.md
node scripts/check-docs-consistency.mjs  # OK

# 范围检查
git status --short   # D pnpm-lock.yaml D pnpm-workspace.yaml; ?? bun.lock .bunfig.toml scripts/fix-electron.mjs apps/desktop/scripts/
```
